...
export default function Pagination({page = 1, onSetPage, total = 0, className, ...other}: {page: number, total: number, onSetPage: Function, className?: React.ComponentProps<'div'>['className']}) {
  if (!total) {
    return;
  }
    ...
    <div className={"flex space-x-1 " + className} {...other}>
      ...
      {Array(total).fill(0).map((_, index) => {
        ...
        if (page === numberPage 
          ...
          || (page === total - 1 && (numberPage === page - 1 || numberPage === page - 2))
          || (page === total && (numberPage === page - 1 || numberPage === page - 2 || numberPage === page - 3))
        ) {
          return (
            <Fragment key={index}>
              {((numberPage === total && page < (total - 3))) && <div  
